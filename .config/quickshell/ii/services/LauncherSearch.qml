...
Singleton {
    ...
    property bool aiConversationActive: false
    property string aiLastQuestion: ""
    property var aiResponseMessage: null
    readonly property bool aiReadyForFollowUp: !aiResponseMessage || (aiResponseMessage.done ?? false)

    function isLikelyQuestion(text) {
        const value = (text || "").trim();
        if (value.length < 4) return false;
        if (value.endsWith("?")) return true;
        return /^(what|why|how|who|when|where|which|can|could|should|would|will|do|does|did|is|are|am|tell me|explain|help me|give me|show me)\b/i.test(value);
    }

    function askAi(text) {
        const question = (text || "").trim();
        if (!question.length) return;
        if (!root.aiReadyForFollowUp) return;
        root.aiConversationActive = true;
        root.aiLastQuestion = question;
        Ai.sendUserMessage(question);
        const responseId = Ai.messageIDs[Ai.messageIDs.length - 1];
        root.aiResponseMessage = Ai.messageByID[responseId] ?? null;
        root.query = "";
    }

    function endInlineAi() {
        root.aiConversationActive = false;
        root.aiLastQuestion = "";
        root.aiResponseMessage = null;
    }
    ...
    property list<var> results: {
        ...
        const aiQuestionResultObject = root.isLikelyQuestion(root.query) ? resultComp.createObject(null, {
            name: root.query.trim(),
            verb: Translation.tr("Ask"),
            type: Translation.tr("AI answer"),
            iconName: "auto_awesome",
            iconType: LauncherSearchResult.IconType.Material,
            keepOpen: true,
            execute: () => root.askAi(root.query)
        }) : null;
        ...
        // A natural-language question should be the first result, while app
        // names and explicit search prefixes keep their existing behavior.
        if (aiQuestionResultObject && !startsWithMathPrefix && !startsWithShellCommandPrefix && !startsWithWebSearchPrefix)
            result.unshift(aiQuestionResultObject);
