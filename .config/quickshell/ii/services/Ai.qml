...
Singleton {
    ...
    property var models: Config.options.policies.ai === 2 ? {} : {
        ...
        "gemini-3.1-flash-lite": aiModelComponent.createObject(this, {
            ...
            "homepage": "https://aistudio.google.com",
            "endpoint": "https://generativelanguage.googleapis.com/v1beta/models/gemini-3.1-flash-lite:streamGenerateContent",
        ...
        "gemini-3.7-flash": aiModelComponent.createObject(this, {
            ...
            "description": Translation.tr("Online | Limited premium fallback model"),
            "homepage": "https://aistudio.google.com",
            "endpoint": "https://generativelanguage.googleapis.com/v1beta/models/gemini-3.7-flash:streamGenerateContent",
    ...
    Process {
        ...
        command: failedModel.length
            ? ["bash", CF.FileUtils.trimFileProtocol(`${Directories.scriptPath}/ai/model-quota-router.sh`), "fallback", "text", failedModel]
            : ["bash", CF.FileUtils.trimFileProtocol(`${Directories.scriptPath}/ai/model-quota-router.sh`), "next", "text"]

        stdout: StdioCollector {
            id: routedModelOutput
        }

        function selectModel(failed = "") {
        ...
        onExited: (exitCode, exitStatus) => {
            ...
            const modelId = routedModels.length ? routedModels[routedModels.length - 1] : "";
            textModelRouter.failedModel = "";
            if (exitCode !== 0 || !root.models[modelId]) {
                root.addMessage(Translation.tr("No AI model quota is currently available."), Ai.interfaceRole);
                return;
            }
            root.setModel(modelId, false, false);
    ...
    Process {
        ...
        function startRequest() {
            ...
            const requestTool = currentModelId.startsWith("gemma-") ? "none" : root.currentTool;
        ...
        function shouldFallback() {
            ...
            return /Error\s+(404|429)|quota exceeded|no longer available/i.test(content);
        }
        ...
        function retryWithFallback() {
            ...
            const failedId = root.idForMessage(requester.message);
            const failedIndex = root.messageIDs.indexOf(failedId);
            if (failedIndex >= 0)
                root.removeMessage(failedIndex);
            textModelRouter.selectModel(failedModel);
        }
    }
