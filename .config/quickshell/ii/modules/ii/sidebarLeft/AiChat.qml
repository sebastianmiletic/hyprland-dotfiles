...
Item {
    ...
    property var quotaCounts: ({})

    function quotaKey(modelId) {
        return modelId === "gemini-3-flash" ? "gemini-3-flash-preview" : modelId;
    }

    function quotaLimit(modelId) {
        const key = root.quotaKey(modelId);
        if (key === "gemma-4-31b-it") return 14400;
        if (key === "gemini-3.5-flash-lite" || key === "gemini-3.1-flash-lite") return 500;
        if (key === "gemini-3-flash-preview" || key === "gemini-3.6-flash" || key === "gemini-3.7-flash") return 20;
        return -1;
    }

    function requestsLeft(modelId) {
        const limit = root.quotaLimit(modelId);
        if (limit < 0) return -1;
        return Math.max(0, limit - (root.quotaCounts[root.quotaKey(modelId)] ?? 0));
    }
    ...
    property var allCommands: [
        ...
        {
            name: "new",
            description: Translation.tr("Start a clean conversation"),
            execute: () => {
                Ai.clearMessages();
                messageInputField.forceActiveFocus();
            }
        ...
        {
            name: "help",
            description: Translation.tr("Show the available assistant commands"),
            execute: () => {
                Ai.addMessage(Translation.tr("**Assistant commands**\n\n- `/new` — start a clean conversation\n- `/model` — choose a model\n- `/tool` — choose search, functions, or none\n- `/attach PATH` — attach a file\n- `/save NAME` and `/load NAME` — manage chats\n- `/temp VALUE` — adjust creativity\n- `/key KEY` — configure the selected provider"), Ai.interfaceRole);
            }
        },
        {
            name: "temp",
...
`, Ai.interfaceRole);
    ...
    Process {
        id: quotaStatusProcess
        command: [
            "bash",
            FileUtils.trimFileProtocol(`${Directories.scriptPath}/ai/model-quota-router.sh`),
            "status"
        ]

        stdout: StdioCollector {
            id: quotaStatusOutput
        }

        function refresh() {
            if (!quotaStatusProcess.running)
                quotaStatusProcess.running = true;
        }

        onExited: (exitCode, exitStatus) => {
            if (exitCode !== 0)
                return;
            const lines = quotaStatusOutput.text.trim().split("\n").filter(line => line.length);
            if (!lines.length)
                return;
            try {
                const status = JSON.parse(lines[lines.length - 1]);
                root.quotaCounts = status.counts ?? {};
            } catch (error) {
                console.warn("[AiChat] Could not parse model quota status:", error);
            }
        }
    }
    ...
    Timer {
        ...
        onTriggered: quotaStatusProcess.refresh()
    }
    ...
    ColumnLayout {
        ...
        Item {
            ...
            Rectangle {
                ...
                RowLayout {
                    ...
                    StatusItem {
                        icon: "auto_awesome"
                        statusText: Ai.getModel()?.name ?? Translation.tr("Assistant")
                        description: Translation.tr("Current assistant model")
                    }
                    StatusSeparator {}
                    StatusItem {
                    ...
                    StatusItem {
                        icon: "edit_square"
                        statusText: ""
                        description: Translation.tr("New conversation")
            ...
            PagePlaceholder {
                ...
                icon: "neurology"
                title: Translation.tr("How can I help?")
                description: Ai.currentModelHasApiKey
                    ? Translation.tr("Ask anything. Shift+Enter adds a new line.")
                    : Translation.tr("Connect an AI provider in Settings, then ask anything.")
        ...
        Rectangle { // Input area
            ...
            implicitHeight: Math.max(inputFieldRowLayout.implicitHeight + 10, 45)
                + modelQuotaIndicator.implicitHeight + spacing
            ...
            RowLayout { // Input field and send button
                ...
                anchors {
                    bottom: modelQuotaIndicator.top
                    left: parent.left
                    right: parent.right
                    bottomMargin: 2
                }
            ...
            RowLayout {
                ...
                StyledText {
                    ...
                    text: {
                        const name = Ai.getModel()?.name ?? Translation.tr("Assistant");
                        const remaining = root.requestsLeft(Ai.currentModelId);
                        return remaining >= 0
                            ? Translation.tr("%1 · %2 requests left").arg(name).arg(remaining)
                            : Translation.tr("%1 · quota unavailable").arg(name);
                    }
                }
            }
            ...
            RowLayout { // Controls
                ...
                ApiInputBoxIndicator {
                    // Model indicator
                    icon: "api"
                    text: Ai.getModel()?.name ?? Translation.tr("Assistant")
                    tooltipText: Translation.tr("Current model: %1\nSet it with /model MODEL").arg(Ai.getModel()?.name ?? Translation.tr("Assistant"))
                }

                ApiInputBoxIndicator {
                    // Tool indicator
                    icon: "service_toolbox"
                    text: (Ai.currentTool || "none").charAt(0).toUpperCase() + (Ai.currentTool || "none").slice(1)
                    tooltipText: Translation.tr("Current tool: %1\nSet it with /tool TOOL").arg(Ai.currentTool || "none")
                }
